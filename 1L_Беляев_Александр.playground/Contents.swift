...
if coefficientA == 0 {
    ...
    if discriminant > 0 {
        ...
        print ("\nПервый корень уравнения (X1) =", firstRoot)
        let secondRoot:Float = (-coefficientB - sqrt(discriminant))/(2*coefficientA)
        print ("Второй корень уравнения (X2) =", secondRoot)
...
if cathetusA <= 0 {
    ...
    print ("\nГипотенуза треугольника (C) =", hypotenuse)

    let square:Float = (cathetusA*cathetusB)/2
    print ("\nПлощадь треугольника (S) =", square)

    let perimeter:Float = cathetusA + cathetusB + hypotenuse
    print ("Периметр треугольника (P) =", perimeter)
}
...
if deposit < 0 {
    ...
    repeat {
        ...
        print ("Сумма вклада на конец",index,"-го года :", deposit)
        index += 1
    } while index < 6
}
